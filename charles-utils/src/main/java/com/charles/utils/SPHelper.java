...
public class SPHelper {
    ...
    public static <T> void putBean(Context context, String key, T value) {
        context.getSharedPreferences(SP_NAME, Context.MODE_PRIVATE).edit().putString(key, JSONObject.toJSONString(value)).commit();
    }

    public static <T> void putBean(Context context, String spName, String key, T value) {
        context.getSharedPreferences(spName, Context.MODE_PRIVATE).edit().putString(key, JSONObject.toJSONString(value)).commit();
    }

    public static <T> T getBean(Context context, String key, Class<T> t) {
        return JSONObject.parseObject(context.getSharedPreferences(SP_NAME, Context.MODE_PRIVATE).getString(key, ""), t);
    }

    public static <T> T getBean(Context context, String spName, String key, Class<T> t) {
        return JSONObject.parseObject(context.getSharedPreferences(spName, Context.MODE_PRIVATE).getString(key, ""), t);
    }
    ...
    public static Long getLong(Context context, String key) {
        return context.getSharedPreferences(SP_NAME, Context.MODE_PRIVATE).getLong(key, 0);
    }
}
