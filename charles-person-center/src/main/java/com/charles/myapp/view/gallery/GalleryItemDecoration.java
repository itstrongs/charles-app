package com.charles.myapp.view.gallery;
...
import com.charles.myapp.ResUtils;
...
public class GalleryItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private void onSetVerticalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        ...
        int itemNewHeight = parent.getHeight() - ResUtils.dpToPx(4 * mPageMargin + 2 * mLeftPageVisibleWidth);

        Log.d("TAG", "itemNewHeight=" + itemNewHeight);

        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int topMargin = position == 0 ? ResUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ResUtils.dpToPx(mPageMargin);
        int bottomMargin = position == itemCount - 1 ? ResUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ResUtils.dpToPx(mPageMargin);
    ...
    private void onSetHoritiontalParams(ViewGroup parent, View itemView, int position, int itemCount) {
        int itemNewWidth = parent.getWidth() - ResUtils.dpToPx(4 * mPageMargin + 2 * mLeftPageVisibleWidth);
        int itemNewHeight = parent.getHeight();

        // 适配第0页和最后一页没有左页面和右页面，让他们保持左边距和右边距和其他项一样
        int leftMargin = position == 0 ? ResUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ResUtils.dpToPx(mPageMargin);
        int rightMargin = position == itemCount - 1 ? ResUtils.dpToPx(mLeftPageVisibleWidth + 2 * mPageMargin) : ResUtils.dpToPx(mPageMargin);
