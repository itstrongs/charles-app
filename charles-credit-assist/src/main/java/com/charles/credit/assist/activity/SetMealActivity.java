...
public class SetMealActivity extends BaseActivity {
    ...
    @OnClick({R.id.img_back, R.id.img_close, R.id.btn_authentication})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_authentication:
                if (initParam()) {
                    mPresenter.createOrder(mActivity, CreditQueryEnum.SET_MEAL.getCode());
                }
                break;
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_PAY && resultCode == RESULT_CODE_PAY_SUCCESS && initParam()) {
            ...
            creditRequest.setQueryType(CreditQueryEnum.SET_MEAL.getCode());
            creditRequest.setRealName(realName);
            creditRequest.setIdCard(idCard);
            creditRequest.setTelephone(telephone);
            mPresenter.queryCredit(mActivity, creditRequest, SetMealResultActivity.class);
        }
    }
}
