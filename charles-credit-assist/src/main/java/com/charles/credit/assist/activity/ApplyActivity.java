...
public class ApplyActivity extends BaseActivity {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_checkup:
                if (initParam()) {
                    mPresenter.createOrder(mActivity, CreditQueryEnum.LOANS_RECORD.getCode());
                }
                break;
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_PAY && resultCode == RESULT_CODE_PAY_SUCCESS && initParam()) {
            ...
            creditRequest.setQueryType(CreditQueryEnum.LOANS_RECORD.getCode());
            creditRequest.setRealName(realName);
            creditRequest.setIdCard(idCard);
            creditRequest.setTelephone(telephone);
            mPresenter.queryCredit(mActivity, creditRequest, ApplyResultActivity.class);
        }
    }
}
