package com.charles.eden.model;

import com.charles.eden.R;

public class ConstantPool {

    public static final String URL_BASE = "http://129.204.231.152:8081/";
//    public static final String URL_BASE = "http://192.168.18.5:8081/";

    public static final String SP_USERNAME = "sp_username";
    public static final String SP_USER_ID = "sp_user_id";
    public static final String SP_NICKNAME = "sp_nickname";
    public static final String SP_PORTRAIT = "sp_portrait";
    public static final String SP_AUTHORIZATION = "sp_authorization";
    public static final String SP_BASE_URL = "sp_base_url";
    public static final String SP_ACTIVITY_UUID = "sp_activity_uuid";
    /** 是否是正式服 */
    public static final String SP_IS_RELEASE = "sp_is_release";

    public static final int[] LIST_TAB_TEXT = new int[]{R.string.tab_text_1, R.string.tab_text_2};
    public static final String ARG_SECTION_NUMBER = "section_number";
}
